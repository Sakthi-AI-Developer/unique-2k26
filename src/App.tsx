    ...
    <main>
      <section id="home" className="hero container">
        <div className="hero-copy">
          ...
          <motion.div {...reveal} className="hero-details"><div><span>ENTRY</span><strong>{siteConfig.fee.replace(' per person', '')}<small> / PERSON</small></strong></div><div><span>TEAM SIZE</span><strong>{siteConfig.teamSize}</strong></div><p><b>ONE TECHNICAL</b> + <b>ONE NON-TECHNICAL</b> EVENT</p></motion.div>
      ...
      <section id="events" className="section container"><motion.div {...reveal} className="section-heading"><div><span className="section-kicker">01 / THE EVENT DIRECTORY</span><h2>Choose your <i>challenge.</i></h2></div><p>Ten ways to make the weekend yours. Select only one event from one category.</p></motion.div><div className="participation-rule"><strong>One event. One category.</strong><span>Registration fee: {siteConfig.fee} · Team size: {siteConfig.teamSize}</span><small>Each team can select ONLY ONE event from Technical OR Non-Technical.</small></div><div className="filter-row" role="group" aria-label="Filter events">{(['ALL', 'TECHNICAL', 'NON-TECHNICAL'] as const).map((filter) => <button key={filter} className={eventFilter === filter ? 'active' : ''} onClick={() => setEventFilter(filter)} aria-pressed={eventFilter === filter}>{filter}</button>)}</div><div className="event-grid">{filteredEvents.map((event, index) => <motion.button initial={{ opacity: 0, y: reduceMotion ? 0 : 14 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: reduceMotion ? 0 : .4, delay: index * .04 }} className="event-card" key={`${eventFilter}-${event.id}`} onClick={() => setSelectedEvent(event)} data-cursor><div className="card-top"><span>{event.id} / {event.category}</span><span className="event-icon">{event.id}</span></div><div className="event-art"><span>{event.title.slice(0, 1)}</span><div className="art-grid" /></div><h3>{event.title}</h3><p><b>{event.type}</b><br />{event.description}</p><div className="card-bottom"><span>{siteConfig.teamSize} · {siteConfig.fee}</span><span>View event <ArrowUpRight size={15} /></span></div></motion.button>)}</div></section>

      <section id="schedule" className="section schedule-section"><div className="container"><motion.div {...reveal} className="section-heading"><div><span className="section-kicker">03 / THE TIMELINE</span><h2>Your day. Your <i>moment.</i></h2></div><p>The complete schedule and event timings will be announced soon.</p></motion.div><motion.div {...reveal} className="schedule-coming"><div className="schedule-status"><span className="status-dot" /> SCHEDULE <b>COMING SOON</b></div><div className="schedule-facts"><div><span>EVENT DAY</span><strong>DATE TO BE ANNOUNCED</strong></div><div><span>TIMINGS</span><strong>TO BE ANNOUNCED</strong></div><div><span>VENUE</span><strong>TO BE ANNOUNCED</strong></div></div><div className="schedule-roadmap" aria-label="Planned event flow, details to be announced"><div><b>01</b><span>EVENTS</span></div><i /><div><b>02</b><span>COMPETITIONS</span></div><i /><div><b>03</b><span>FINALS</span></div><i /><div><b>04</b><span>PRIZE CEREMONY</span></div></div><small className="schedule-note">A single-day symposium flow. Official sequence and timings will be shared after confirmation.</small></motion.div></div></section>
